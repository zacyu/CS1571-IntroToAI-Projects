#!/usr/bin/env python3
"""
CS 1571 Project 3: FOL Inference - Iterative Forward Chaining
Zac Yu (zhy46@)
"""
...
class FolIfcSystem(object):
    """A first-order-logic system using iterative forward chaining rules"""
    predicate_symbols = []
    constant_symbols = []
    rules = []
    facts = []
    ...
    def add_fact(self, fact, source=False):
        ...
        self.fc_counter += 1
        if fact.predicate_id not in self.relevant_rules:
            return True
        for rule_id in self.relevant_rules[fact.predicate_id]:
            rule = self.rules[rule_id]
            for i, atom in enumerate(rule.lhs):
                bindings = self.unify(atom, fact)
                if bindings is not False:
                    self.find_and_infer(rule_id, rule.lhs[:i] + rule.lhs[i + 1:],
                                        rule.rhs, bindings)
        return True
    ...
    def add_rule(self, rule):
        ...
        for atom in rule.lhs:
            pid = atom.predicate_id
            if pid not in self.relevant_rules:
                self.relevant_rules[pid] = set()
            self.relevant_rules[pid].add(len(self.rules))
...
def main():
    ...
    fol_ifc = FolIfcSystem()

    print('-' * 36 + ' Given ' + '-' * 36)
    for line in args.config_file:
        components = line.split()
        if len(components) > 2:
            if not fol_ifc.add_rule(fol_ifc.parse_rule(components)):
                raise SystemExit('error: invalid input line: ' + line)
        elif len(components) == 1:
            if not fol_ifc.add_fact(fol_ifc.parse_atom(components[0])):
                raise SystemExit('error: invalid input line: ' + line)
        else:
            if components[0] != 'PROVE':
                raise SystemExit('error: invalid input line: ' + line)
            query = fol_ifc.parse_atom(components[1])
            print('-' * 36 + ' Promt ' + '-' * 36)
            print('Show that', fol_ifc.format_atom(query))
            print('-' * 36 + ' Proof ' + '-' * 36)
            result = fol_ifc.ask(query)
            if result is False:
                print('Failed')
            else:
                print('Q.E.D.')
    ...
    print('-' * 37 + ' Raw ' + '-' * 37)
    print(result)
    for fid, fact in enumerate(fol_ifc.facts):
        if fid in fol_ifc.fact_srcs:
            print(fol_ifc.format_atom(fact))
        if fact == query:
            break
